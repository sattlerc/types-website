...
Edit `book-of-abstracts/book-of-abstracts.tex` to your liking.
The included files (`\input{committees}` and `\input{core}`) are generated by running:

```shell
cabal run book-of-abstracts
```
...
```shell
./generate_calendar.py >types20XX.ics
```

This uses the data files in `data`.

Note that this requires the `icalendar` Python package.
If Python complains about this package missing, you can set it up in a local virtual environment as follows:
